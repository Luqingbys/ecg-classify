...
def load_mit_db(DS, winL, winR, do_preprocess, maxRR, use_RR, norm_RR, compute_morph, db_path, reduced_DS, leads_flag):
    ...
    if os.path.isfile(features_labels_name):
        ...
        if os.path.isfile(mit_pickle_name):
            ...
            # pickle.dump(my_db, f, 2) # 序列化，将对象my_db写到文件f中，2表示新的二进制协议
            joblib.dump(my_db, f) # 序列化，将对象my_db写到文件f中，2表示新的二进制协议
            f.close
        ...
        joblib.dump([features, labels, patient_num_beats], f)
        f.close
...
def load_signal(DS, winL, winR, do_preprocess):
    ...
    pathDB = 'G:\深度学习\医疗\ecg-classification-master\dataset'
    # DB_name = 'mitdb'
    DB_name = '\\archive'
    fs = 360
    jump_lines = 1
    ...
    # fRecords、fAnnotations是.csv(需要提取的特征)、.txt(标签)的文件名列表
    fRecords = list()
    fAnnotations = list()

    # lst: list = os.listdir(pathDB + DB_name + "/csv")
    lst: list = os.listdir(pathDB + DB_name) # 返回指定目录下的文件名列表
    lst.sort()
